...
def evaluateCamPath():
    ...
    for i in range(int(startFrame), int(endFrame), int(bakeSteps * timeDetailFactor)):
        ...
        camLocPos = cmds.xform(cmds.ls(sl=1), query=True, translation=True, worldSpace=True)
        camLocRot = cmds.xform(cmds.ls(sl=1), query=True, rotation=True, worldSpace=True)
...
def camFromSelection():
    cam = cmds.ls(sl=1)
    # cmds.textFieldButtonGrp( camField, edit=True, text=str(cam))
...
# camField=cmds.textFieldButtonGrp( label='Camera', text=cam, buttonLabel='from selection',bc='camFromSelection()',cw3=[60,160,50] )
cmds.text(label='', w=windowW, h=5)
cmds.button("Import evaluation rig", h=30, w=windowW, c='importEvalRig()')
cmds.text(label='', w=windowW, h=5)
