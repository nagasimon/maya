...
sctBxLocator = 'BoxScatter_v02_boxScatterLocator'
sctNode = 'BoxScatter_v02_voxelGrid'
sctBox = 'BoxScatter_v02_polySphere1'
...
timeConvert = 'InstancerFromCurve_RandomRotateY_v01_timeToUnitConversion1'
...
def evaluateCamPath():
    ...
    try:
        ...
        cmds.file("/uvfx/Projects/Promesse_de_laube/Test/Sequence_202/BoxScatter_v02.ma", removeReference=True)
    except:
        print 'bad ending'
...
def generateRainRig():
    ...
    cmds.setAttr('RainScatterSample_v01_rainInstanceAGroup'+'.CylinderWidth',pointSeparation)

    # setup the animation

    cmds.setAttr(instancerNode + '.levelOfDetail', 1)
    cmds.setAttr(instancerNode + '.MaxRandom', pointSeparation)
    cmds.setAttr(instancerNode + '.MinRandom', -pointSeparation)
    cmds.connectAttr('time1.outTime', timeConvert + '.input')

    # Setup the display curve as point
    cmds.setAttr(cmds.listRelatives(curveSource, shapes=True)[0] + ".dispGeometry", 0)
    cmds.setAttr(cmds.listRelatives(curveSource, shapes=True)[0] + ".dispCV", 1)


def connectToCentral(centralObj, sourceObj):
    sel = cmds.ls(sl=1)
    attributeList = cmds.listAttr(sourceObj, userDefined=True)
    cmds.select(centralObj, replace=True)
    for att in attributeList:
        attType = cmds.getAttr(sourceObj + '.' + att, type=True)
        cmds.addAttr(centralObj, attributeType=attType, longName=att, k=True,
                     defaultValue=cmds.getAttr(sourceObj + '.' + att))
        cmds.connectAttr(centralObj + '.' + att, sourceObj + '.' + att)
...
def importBoxScatterRig():
    ...
    cmds.file("/uvfx/Projects/Promesse_de_laube/Test/Sequence_202/BoxScatter_v02.ma",
              rpr="BoxScatter_v02",
              r=True,
              type="mayaAscii",
              ignoreVersion=True,
...
def parameterChange():
    global bakeSteps, boxScl, pointSeparation
    ...
    pointSeparation = int(cmds.floatFieldGrp(pointSeparationField, query=True, value=True)[0])

    # Setup the options
    cmds.setAttr(sctNode + '.voxelScale', pointSeparation)
...
def setupOnLocator():
    centralLocator = cmds.spaceLocator(name='CentralRainController')
    cmds.setAttr(centralLocator[0] + '.outlinerColor', 1, 1, 0, type='double3')
    cmds.setAttr(centralLocator[0] + '.useOutlinerColor', 1)
    connectToCentral(centralLocator[0], 'RainScatterSample_v01_rainInstanceAGroup')
    connectToCentral(centralLocator[0], instancerNode)
    cmds.setAttr(centralLocator[0] + '.animationOffset', cmds.playbackOptions(query=True, minTime=True))
...
SingingInTheRain = cmds.window(windowID, title=windowID, resizeToFitChildren=True, sizeable=True, w=windowW, h=170)
...
camField = cmds.textFieldButtonGrp(label='Camera', text='select the camera', buttonLabel='from selection',
                                   bc='camFromSelection()',
                                   cw3=[60, 160, 50])
cmds.text(label='', w=windowW, h=5)
...
pointSeparationField = cmds.floatFieldGrp(numberOfFields=1, label='Point separation', value1=pointSeparation,
                                       cc='parameterChange()')
...
cmds.text(label='', w=windowW, h=10)
cmds.text(label='Centralised Setup', w=windowW, h=15, bgc=colorLevelA)
cmds.text(label='', w=windowW, h=5)
cmds.button("Setup on Locator", h=30, bgc=[0.5, 0.2, 0.2], w=windowW, c='setupOnLocator()')

cmds.setParent('..')

# rainSpeedField = cmds.floatFieldGrp(numberOfFields=1, label='Rain speed', value1=100, cc='rainParameterChange()',cl3=['left', 'left', 'left'])
